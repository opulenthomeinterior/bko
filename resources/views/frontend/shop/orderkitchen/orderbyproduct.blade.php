            ...
            <div class="row">
                    ...
                    <div class="col-lg-6 col-12">
                        <h2 class="fs-4 text-dark text-uppercase fw-bolder">
                         ...
                         <small class="fs-6 fw-bold text-dark">{{ $product->dimensions }}</small>
                         <h2 class="fw-bold text-warning">£{{ $product->price }}</h2>
                         <div class="d-flex gap-5">
                            <div class="d-flex flex-column">
                                <p class="fs-6 fw-bold">Product Code </p>
                                <p class="fs-6 border border-warning p-2">{{$product->product_code}}</p>
                            </div>
                            @if($product->ParentCategory?->slug == 'doors' || $product->ParentCategory?->slug == 'handles')
                            <div class="d-flex flex-column">
                                <p class="fs-6 fw-bold">Order a Free Sample </p>
                                <a href="{{ route('checkout')}}" class="btn border-1 border-warning p-2">Checkout</a>
                                        ...
                                        <div class="ltn__product-details-menu-2 my-0 d-flex justify-content-start align-items-center gap-3">
                                            <div class="cart-plus-minus">
                                                <div class="dec qtybutton" id="dec-btn{{ $product->id }}">-</div>
                                                    <input type="text" value="0" name="qtybutton" id="quantity{{ $product->id }}" class="cart-plus-minus-box">
                                                <div class="inc qtybutton">+</div>
                                            </div>
                                            <div>
                                                <button class="theme-btn-1 btn btn-effect-1">
                                                    <i class="fas fa-shopping-cart"></i>
                                                    <span onclick="inputQty(document.querySelector('#quantity{{ $product->id }}').value, '{{ $product->id }}', '{{ $product->product_code }}', '{{ $product->full_title }}', {{ $product->price }}, {{ $product->discounted_price }}, {{ $product->discounted_percentage ?? 0 }}, '{{ $product->ParentCategory->slug }}','{{ $product->image_path }}')">ADD TO CART</span>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
